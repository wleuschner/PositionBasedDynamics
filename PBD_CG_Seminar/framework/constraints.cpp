...
void solveDistanceConstraint(const QVector3D& p1,const QVector3D& p2, float w1, float w2,float l0,float compression_k,float stretch_k,QVector3D& dp1,QVector3D& dp2)
{
...
void solveCollisionSphereConstraint(const QVector3D& point,float mass,const QVector3D& colPos,float r,QVector3D& dp1)
{
    QVector3D grad = QVector3D(1/(2*point.x()-2*colPos.x()),1/(2*point.y()-2*colPos.y()),1/(2*point.z()-2*colPos.z()));
    float dist = (point-colPos).length();
    dp1 = ((-dist+r)/(mass*grad.length())) * grad;
}
