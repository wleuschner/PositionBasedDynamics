...
bool Model::load(std::string path)
{
    ...
    for(unsigned int j=0;j</*scene->mNumMeshes*/1;j++)
    {
        ...
        for(int i=0;i<mesh->mNumFaces;i++)
        {
            ...
            e2 = Edge(f.v1,f.v3);
            e3 = Edge(f.v2,f.v3);
            if(edges.indexOf(e1)==-1)
            {
                edges.push_back(e1);
            }
            if(edges.indexOf(e2)==-1)
            {
                edges.push_back(e2);
            }
            if(edges.indexOf(e3)==-1)
            {
                edges.push_back(e3);
            }
        }
...
{
    vbo.bind();
    vbo.write(0,vertices.constData(),sizeof(Vertex)*vertices.length());
    bind();
}
...
void Model::draw(QOpenGLShaderProgram* shader)
{
    ...
    shader->setUniformValue("doubleFaced",doubleFaced);
...
Model* Model::createPlaneXZ(float width,float height,int xPatches,int zPatches)
{
    ...
    for(int y=0;y<zPatches-1;y++)
    {
        for(int x=0;x<xPatches-1;x++)
        {
            ...
            e5 = Edge(f2.v1,f2.v3);
            e6 = Edge(f2.v2,f2.v3);
            if(model->edges.indexOf(e1)==-1)
            {
                model->edges.push_back(e1);
            }
            if(model->edges.indexOf(e2)==-1)
            {
                model->edges.push_back(e2);
            }
            if(model->edges.indexOf(e3)==-1)
            {
                model->edges.push_back(e3);
            }
            if(model->edges.indexOf(e4)==-1)
            {
                model->edges.push_back(e4);
            }
            if(model->edges.indexOf(e5)==-1)
            {
                model->edges.push_back(e5);
            }
            if(model->edges.indexOf(e6)==-1)
            {
...
Model* Model::createPlaneXY(float width,float height,int xPatches,int yPatches)
{
    ...
    for(int i=0;i<model->position.length();i++)
    {
        ...
        if(i>yPatches*xPatches-xPatches && i<yPatches*xPatches)
        {
            v.setMass(0.0f);
        }
        else
