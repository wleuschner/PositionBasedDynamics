...
void Solver::solve()
{
    ...
    for(QList<Entity>::iterator e=ballonList.begin();e!=ballonList.end();e++)
    {
        ...
        for(int i=0;i<20;i++)
        {
            ...
            solveTetrahedralConstraint(positions,faces,facemap,volume,2.0,dp);
            ...
            for(QVector<Edge>::iterator j=edges.begin();j!=edges.end();j++)
            {
                ...
                solveDistanceConstraint(p1,p2,m->getOriginalEdgeLength(j->v1,j->v2),k_stretch,k_stretch,dp1,dp2);
                p1 += dp1;
                p2 += dp2;
    ...
    for(QList<Entity>::iterator e=softList.begin();e!=softList.end();e++)
    {
        ...
        {
            //Solve Distance Constraint
            float k_stretch = 1 - pow(1-0.8,1.0/solverLoops);
